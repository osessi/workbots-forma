...
- For images: If an image exists in the original slide, preserve its exact URL from the screenshot. Do NOT add placeholder text like "PLACE HERE", "Hero Image", or any other placeholder content. If there's no image, simply omit the img element entirely.
- NEVER add placeholder text like "PLACE HERE", "Hero Image", "Image placeholder" etc. Leave image containers empty or use the actual image URL from the slide.
