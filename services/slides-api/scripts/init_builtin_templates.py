...
This script creates professional PPTX template files for the built-in templates
...
Run this once to initialize the templates, then they'll be available in Smart Templates.
You can later replace these with better designed PPTX files via the admin interface.
"""
...
def create_title_slide(prs: Presentation, scheme: dict, slide_num: int = 0):
...
def create_content_slide(prs: Presentation, scheme: dict, slide_num: int = 1):
...
def create_bullets_slide(prs: Presentation, scheme: dict, slide_num: int = 2):
...
def create_image_content_slide(prs: Presentation, scheme: dict, slide_num: int = 3):
...
def create_closing_slide(prs: Presentation, scheme: dict, slide_num: int = 4):
...
def create_template_pptx(template_id: str, scheme: dict, output_dir: str) -> str:
    ...
    # Create slides
    create_title_slide(prs, scheme, 0)
    create_content_slide(prs, scheme, 1)
    create_bullets_slide(prs, scheme, 2)
    create_image_content_slide(prs, scheme, 3)
    create_closing_slide(prs, scheme, 4)
...
def init_builtin_templates():
    """Initialize all built-in templates as PPTX files in Smart Templates"""

    # Create templates directory
    templates_dir = os.path.join(get_app_data_directory(), "smart_templates")
    os.makedirs(templates_dir, exist_ok=True)

    print("Initializing built-in templates...")

    with Session(engine) as session:
        for template_id, scheme in TEMPLATE_SCHEMES.items():
            # Check if already exists
            existing = session.query(PptxTemplateModel).filter(
                PptxTemplateModel.name == scheme["name"],
                PptxTemplateModel.category == "builtin"
            ).first()

            if existing:
                print(f"  - {scheme['name']}: Already exists, skipping")
                continue
    ...
    print("\nBuilt-in templates initialized successfully!")
    print("You can now replace them with better designed PPTX files via Smart Templates admin.")
