...
// Helper pour formater la date "Membre depuis"
...
export default function MonComptePage() {
  ...
  // Récupérer les stats (sessions count)
  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch("/api/dashboard/stats");
      if (res.ok) {
        const data = await res.json();
        setSessionsCount(data.sessions?.total || 0);
      }
    } catch (error) {
        ...
        <div className="px-6 pb-6">
          ...
          <div className="grid grid-cols-3 gap-4 mt-6 pt-6 border-t border-gray-100 dark:border-gray-800">
            ...
            <div className="text-center border-x border-gray-100 dark:border-gray-800">
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{sessionsCount}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">Sessions</p>
            </div>
            <div className="text-center">
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{formatMemberSince(user.createdAt)}</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  Géré via votre compte de connexion
                </p>
              </div>
              <Link
                href="/reset-password"
