...
interface FormationImportData {
  ...
  modules: ModuleData[];
  // Correction 348: Champs certification pour le tableau croisé RS
...
const initialFormData: FormationImportData = {
  ...
  modules: [],
  // Correction 348: Valeurs par défaut certification
  isCertifiante: false,
  numeroFicheRS: "",
  lienFranceCompetences: "",
};

// Correction 349: Textes fixes identiques à ceux utilisés dans le parcours IA
// Ces textes garantissent la cohérence entre les deux modes de création
...
export default function ImportFormationPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const filteredObjectifs = formData.objectifs.filter((obj) => obj.trim() !== "");

      // Filtrer les modules avec des contenus vides
      const filteredModules = formData.modules
        ...
        .map((m, i) => ({
          ...
          contenu: {
            items: m.contenu.filter((c) => c.trim() !== ""),
            objectifs: m.objectifs.filter((o) => o.trim() !== ""),
          },
        }));

      // Correction 349: Générer le contenu texte de la formation à partir des modules
      // Ce champ est utilisé par le wizard pour l'affichage et la génération d'évaluations
      const contenuFormation = filteredModules.map((m, i) => {
        const items = m.contenu.items.length > 0
          ? m.contenu.items.map(item => `- ${item}`).join('\n')
          : '';
        return `Module ${i + 1} – ${m.titre}\n${items}`;
      }).join('\n\n');

      // Déterminer le mode de création (IMPORT si fichier uploadé, MANUAL sinon)
      const creationMode = uploadedFile ? "IMPORT" : "MANUAL";
      ...
      const payload = {
        ...
        fichePedagogique: {
          ...
          objectifGeneral: formData.description.trim(),
          publicVise: formData.publicVise.trim(),
          prerequis: formData.prerequis.trim(),
          accessibilite: formData.accessibilite.trim(),
          ...
          // Correction 349: Durée formatée pour l'affichage
          duree: formData.dureeHeures
            ? `${formData.dureeHeures} heures${formData.dureeJours ? ` (${formData.dureeJours} jours)` : ''}`
            : formData.dureeJours
              ? `${formData.dureeJours} jours`
              : '',
          ...
          // Correction 349: Contenu formaté pour l'affichage et la génération d'évaluations
          contenu: contenuFormation,
          // Correction 349: Stocker aussi les modules dans la fiche pour la compatibilité
          modules: filteredModules.map((m, i) => ({
            titre: m.titre,
            ordre: i + 1,
            contenu: m.contenu.items,
            objectifs: m.contenu.objectifs,
          })),
          // Correction 349: Textes fixes pour la cohérence avec le mode IA
        ...
        contexteData: {
          ...
          tarifParticulier: formData.tarifParticulier,
          description: formData.description.trim(),
          // Correction 348: Inclure les données de certification
          isCertifiante: formData.isCertifiante,
          numeroFicheRS: formData.numeroFicheRS.trim(),
          lienFranceCompetences: formData.lienFranceCompetences.trim(),
        },
      ...
      router.push(`/create?id=${newFormation.id}`);
    } catch (err) {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <button
            onClick={() => {
              setMode("upload");
              setError(null); // Correction 338: Réinitialiser l'erreur lors du changement de mode
            }}
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              Uploadez votre fiche pédagogique existante (PDF ou Word). Les informations sont extraites, puis le contenu peut être ajusté, amélioré et complété grâce à l&apos;IA.
            </p>
          ...
          >
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              Remplissez le formulaire pour saisir votre formation manuellement, en gardant le contrôle sur chaque information.
            </p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          ...
          {/* Correction 348: Section Certification RS/RNCP */}
          <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">
              Certification (Qualiopi IND 7)
            </h2>
            <div className="space-y-4">
              {/* Toggle formation certifiante */}
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  checked={formData.isCertifiante}
                  onChange={(e) => updateFormData("isCertifiante", e.target.checked)}
                  className="w-4 h-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500 dark:border-gray-600"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">
                  Formation certifiante (inscrite au RS/RNCP)
                </span>
              </label>

              {/* Champs conditionnels si certifiante */}
              {formData.isCertifiante && (
                <div className="space-y-3 pt-2 border-t border-gray-100 dark:border-gray-700">
                  <div>
                    ...
                    <input
                      type="text"
                      value={formData.numeroFicheRS}
                      onChange={(e) => updateFormData("numeroFicheRS", e.target.value)}
                      placeholder="Ex: 5558"
                      className="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder:text-gray-500"
                    />
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                      Numéro de la fiche au Répertoire Spécifique (RS) ou RNCP
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                    ...
                    <input
                      type="url"
                      value={formData.lienFranceCompetences}
                      onChange={(e) => updateFormData("lienFranceCompetences", e.target.value)}
                      placeholder="https://www.francecompetences.fr/..."
                      className="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 text-gray-800 placeholder:text-gray-400 focus:border-brand-300 focus:outline-none focus:ring-2 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-800 dark:text-white dark:placeholder:text-gray-500"
                    />
                  </div>
                </div>
