...
import { useIntervenantPortal, IntervenantPortalProvider } from "@/context/IntervenantPortalContext";
...
function LoginContent() {
  ...
  const { login, isAuthenticated, isLoading: contextLoading } = useIntervenantPortal();
  ...
  useEffect(() => {
    if (!contextLoading && isAuthenticated) {
      router.push("/intervenant/accueil");
    }
  ...
  const validateMagicToken = async (magicToken: string) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/intervenant/verify-token?token=${magicToken}`);
      if (res.ok) {
  ...
  const sendCode = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/intervenant/send-code", {
        method: "POST",
  ...
  const verifyCode = async () => {
    ...
    try {
      const res = await fetch("/api/intervenant/verify-code", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  // Couleur primaire (utiliser celle de l'organisation ou emerald par défaut)
  const primaryColor = brandOrg?.primaryColor || "#10b981";
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-emerald-50 dark:from-gray-950 dark:via-gray-900 dark:to-gray-950 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
          ...
          >
            ...
            <h1 className="text-2xl font-bold text-white mb-1">
              {isCustomDomain && brandOrg ? brandOrg.name : "Espace Intervenant"}
            </h1>
              ...
              {step === "code" && (
                ...
                >
                  ...
                  <div className="flex justify-center gap-2">
                    {code.map((digit, index) => (
                      <input
                        ...
                        className="w-12 h-14 text-center text-xl font-bold border-2 border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 transition-all"
                        disabled={loading}
                      />
                    ))}
                  </div>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-3 px-4 disabled:bg-gray-300 dark:disabled:bg-gray-700 text-white font-medium rounded-xl transition-colors"
                    style={{
                      backgroundColor: loading || code.join("").length !== 6 ? undefined : primaryColor,
                    }}
                  >
                    {loading ? (
                      <Loader2 className="w-5 h-5 animate-spin" />
        ...
        <div className="text-center text-sm text-gray-500 dark:text-gray-400 mt-6">
          {isCustomDomain && brandOrg ? (
            <>
              <p className="mb-1">Vous n&apos;êtes pas intervenant ?</p>
...
export default function IntervenantLoginPage() {
  return (
