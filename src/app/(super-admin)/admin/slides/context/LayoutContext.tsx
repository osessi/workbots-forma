...
import { setLayoutLoading } from "@/store/slides-store/slices/presentationGeneration";
...
export const LayoutProvider: React.FC<{
        ...
        for (const fileName of template.files) {
          try {
            ...
            const module = await import(
              `@/components/slides/presentation-templates/${template.templateID}/${file}`
            );
  ...
  const loadLayouts = async () => {
    try {
      ...
      const templateResponse = await fetch("/api/slides/templates");
  ...
  const LoadCustomLayouts = async () => {
    ...
    try {
      const customTemplateResponse = await fetch(
        `/api/slides/v1/ppt/template-management/summary`,
        {
          headers: {
            ...getHeader(),
      ...
      for (const templateInfo of customTemplates) {
        ...
        const customLayoutResponse = await fetch(
          `/api/slides/v1/ppt/template-management/get-templates/${presentationId}`,
          {
            headers: {
              ...getHeader(),
              ...(token ? { Authorization: `Bearer ${token}` } : {}),
  ...
  const getLayout = (
    ...
    const Layout = dynamic(
      () => import(`@/components/slides/presentation-templates/${fileInfo.templateID}/${file}`),
      {
        loading: () => (
