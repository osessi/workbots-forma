...
export const useLayoutSaving = (
  ...
  // Optional: ID of the original PPTX template for high-quality export
  originalPptxTemplateId?: string | null
) => {
  ...
  const convertSlideToReact = async (slide: ProcessedSlide, presentationId: string, FontUrls: string[]) => {
    ...
    while (retryCount < maxRetries) {
      try {
        const response = await fetch("/api/slides/v1/ppt/html-to-react/", {
          method: "POST",
          headers: {
  ...
  const saveLayout = useCallback(async (layoutName: string, description: string): Promise<string | null> => {
    ...
    try {
      ...
      // Include the original PPTX template ID for high-quality export
      ...
      // Link to original PPTX template if available (for high-quality export)
      if (originalPptxTemplateId) {
        templateMetadata.pptx_template_id = originalPptxTemplateId;
      }

      await fetch("/api/slides/v1/ppt/template-management/templates", {
        method: "POST",
      ...
      const saveResponse = await fetch(
        "/api/slides/v1/ppt/template-management/save-templates",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
