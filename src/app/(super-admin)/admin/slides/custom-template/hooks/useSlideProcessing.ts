...
import { ApiResponseHandler } from "@/app/(super-admin)/admin/slides/services/api/api-error-handler";
...
export const useSlideProcessing = (
  ...
  // Store the original PPTX template ID for high-quality export
  const [originalPptxTemplateId, setOriginalPptxTemplateId] = useState<string | null>(null);
  ...
  const processSlideToHtml = useCallback(
    async (slide: SlideData, index: number) => {
      ...
      try {
        const htmlResponse = await fetch("/api/slides/v1/ppt/slide-to-html", {
          method: "POST",
          headers: {
  ...
  const processFile = useCallback(async () => {
    ...
    try {
      ...
      if (isPdf) {
        ...
        const pdfResponse = await fetch("/api/slides/v1/ppt/pdf-slides/process", {
          method: "POST",
          body: formData,
        });
        ...
        const pptxResponse = await fetch("/api/slides/v1/ppt/pptx-slides/process", {
          method: "POST",
          body: formData,
        });
        slidesResponseData = await ApiResponseHandler.handleResponse(
      ...
      // Save the original PPTX file for high-quality export later
      // This preserves the original design when exporting
      if (isPptx) {
        try {
          const templateFormData = new FormData();
          templateFormData.append("file", selectedFile);
          templateFormData.append("name", selectedFile.name.replace(/\.pptx$/i, ""));
          templateFormData.append("description", "Custom template imported from PPTX");
          templateFormData.append("category", "custom");

          const saveTemplateResponse = await fetch("/api/slides/pptx-templates", {
            method: "POST",
            body: templateFormData,
          });

          if (saveTemplateResponse.ok) {
            const templateData = await saveTemplateResponse.json();
            setOriginalPptxTemplateId(templateData.id);
            console.log("Original PPTX saved for high-quality export:", templateData.id);
          } else {
            console.warn("Could not save original PPTX for high-quality export - will use basic export");
          }
        } catch (saveError) {
          console.warn("Failed to save original PPTX:", saveError);
          // Non-blocking - continue with the flow even if saving original fails
        }
      }
  ...
  return {
    ...
    processSlideToHtml,
    retrySlide,
    // ID of the original PPTX template saved for high-quality export
    originalPptxTemplateId,
  };
}; 