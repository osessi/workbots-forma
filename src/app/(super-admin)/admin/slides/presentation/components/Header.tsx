...
import {
  ...
  PopoverContent,
  PopoverTrigger,
} from "@/components/slides/ui/popover";
...
import { RootState } from "@/store/slides-store/store";
import { toast } from "sonner";


import Announcement from "@/components/slides/Announcement";
import { PptxPresentationModel } from "@/types/slides/pptx_models";
...
import ToolTip from "@/components/slides/ToolTip";
import { clearPresentationData } from "@/store/slides-store/slices/presentationGeneration";
import { clearHistory } from "@/store/slides-store/slices/undoRedoSlice";
...
const Header = ({
  ...
  const handleReGenerate = () => {
    ...
    router.push(`/admin/slides/presentation?id=${presentation_id}&stream=true`);
  };
  ...
  const ExportOptions = ({ mobile }: { mobile: boolean }) => (
    <div className={`space-y-2 max-md:mt-4 ${mobile ? "" : "bg-white"} rounded-lg`}>
      <Button
        ...
        variant="ghost"
        className={`pb-4 border-b rounded-none border-gray-300 w-full flex justify-start text-gray-700 hover:text-gray-900 ${mobile ? "bg-white py-6 border-none rounded-lg" : ""}`} >
      ...
      <Button
        ...
        variant="ghost"
        className={`w-full flex justify-start text-gray-700 hover:text-gray-900 ${mobile ? "bg-white py-6" : ""}`}
      >
        <FileSpreadsheet className="w-7 h-7 mr-2" />
        Export as PPTX
      </Button>
      ...
      <div style={{
        ...
        <Popover open={open} onOpenChange={setOpen} >
          <PopoverTrigger asChild>
            <Button className={`border py-5 text-gray-900 font-bold rounded-[32px] transition-all duration-500 hover:border hover:bg-gray-700 hover:text-white w-full ${mobile ? "" : "bg-white"}`}>
  ...
  return (
    <>
      ...
      <div
        ...
        className="bg-gray-900 w-full shadow-lg sticky top-0 ">

        <Announcement />
        <Wrapper className="flex items-center justify-between py-1">
          <Link href="/admin/slides/dashboard" className="min-w-[162px] flex items-center gap-2">
            <img
              className="h-12"
