import {
  getHeader,
} from "@/app/(super-admin)/admin/slides/services/api/header";
...
export class DashboardApi {
  ...
  static async getPresentations(): Promise<PresentationResponse[]> {
    try {
      const response = await fetch(
        `/api/slides/v1/ppt/presentation/all`,
        {
          method: "GET",
        }
      );
  ...
  static async getPresentation(id: string) {
    try {
      const response = await fetch(
        `/api/slides/v1/ppt/presentation/${id}`,
        {
          method: "GET",
        }
      );
  ...
  static async deletePresentation(presentation_id: string) {
    try {
      const response = await fetch(
        `/api/slides/v1/ppt/presentation/${presentation_id}`,
        {
          method: "DELETE",
          headers: getHeader(),
        }
