...
import { Card } from "@/components/slides/ui/card";
import { Button } from "@/components/slides/ui/button";
import { ArrowLeft, Home, Trash2, Code, Save, X, Pencil } from "lucide-react";
import { useLayout } from "@/app/(super-admin)/admin/slides/context/LayoutContext";
...
import { Sheet, SheetContent, SheetFooter, SheetHeader, SheetTitle } from "@/components/slides/ui/sheet";
...
const GroupLayoutPreview = () => {
  ...
  useEffect(() => {
    const loadCustomLayouts = async () => {
      ...
      try {
        const res = await fetch(`/api/slides/v1/ppt/template-management/get-templates/${presentationId}`, {
          headers: getHeader(),
        });
        if (!res.ok) return;
  ...
  const deleteLayouts = async () => {
    refetch();
    router.back();
    const response = await fetch(`/api/slides/v1/ppt/template-management/delete-templates/${presentationId}`, {
      method: "DELETE",
      headers: getHeader(),
    });
    if (response.ok) {
      router.push("/admin/slides/template-preview");
    }
  }
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await fetch(`/api/slides/v1/ppt/template-management/save-templates`, {
        method: "POST",
        headers: getHeader(),
