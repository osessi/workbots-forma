// ===========================================
// API ROUTE: GENERATION ATELIER
// ===========================================

import { NextRequest, NextResponse } from "next/server";
import { z } from "zod";
import { generateAtelier, isAIConfigured } from "@/lib/ai";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generer l'atelier
    const result = await generateAtelier(validationResult.data);

    if (!result.success) {
      console.error("Erreur generation atelier:", result.error);
      return NextResponse.json(
    ...
    // Log pour monitoring
    console.log("Atelier genere:", {
      userId: user.id,
      module: validationResult.data.moduleTitre,
    ...
    console.error("Erreur API generate-atelier:", error);
    return NextResponse.json(
