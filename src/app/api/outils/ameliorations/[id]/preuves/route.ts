// ===========================================
// API PREUVES AMELIORATION - Upload/Delete
// Qualiopi IND 32 - Pièces jointes des améliorations
// ===========================================
...
export async function POST(
  ...
  try {
    const { id: ameliorationId } = await params;
    ...
    // Vérifier que l'amélioration appartient à l'organisation
    ...
    if (!amelioration) {
      return NextResponse.json(
        { error: "Amélioration non trouvée" },
        { status: 404 }
      );
    }
    ...
    const path = `ameliorations-preuves/${sanitizedOrgId}/${ameliorationId}/${fileId}.${extension}`;
    ...
    const existingPieces = (amelioration.piecesJointes as PieceJointe[] | null) || [];
    ...
    await prisma.actionAmelioration.update({
      where: { id: ameliorationId },
      data: {
