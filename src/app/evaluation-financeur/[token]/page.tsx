...
export default function EvaluationFinanceurPage() {
  ...
  useEffect(() => {
    async function loadEvaluation() {
      try {
        const res = await fetch(`/api/evaluation-financeur/${token}`);
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-emerald-100 p-4">
        <div className="text-center max-w-md bg-white rounded-2xl shadow-xl p-8">
          ...
          <p className="text-gray-600">
            Votre évaluation a été enregistrée avec succès. Elle nous aide à améliorer la qualité de nos services.
          </p>
        </div>
      </div>
    );
  ...
  const textQuestions = evaluation.questions.filter(q => q.type === "text");
        ...
        <div className="max-w-md mx-auto">
          <div className="bg-white rounded-2xl shadow-xl p-8">
            <div className="text-center mb-6">
              ...
              <h1 className="text-xl font-bold text-gray-900">Évaluation Financeur</h1>
            ...
            <div className="bg-gray-50 rounded-xl p-4 mb-6">
              <p className="text-sm font-medium text-gray-700 mb-1">Financeur</p>
              <p className="text-lg font-semibold text-gray-900">{evaluation.financeur.nom}</p>
              <p className="text-sm text-gray-500">{evaluation.financeur.type}</p>
            </div>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          ...
          <div className="mt-4 flex items-center gap-4 text-sm text-gray-600">
            <span>{evaluation.financeur.nom}</span>
            <span>|</span>
            <span>{evaluation.session.nombreParticipants} stagiaire(s)</span>
          </div>
        </div>

        {/* Rating Questions */}
        <div className="bg-white rounded-2xl shadow-lg p-6 mb-6">
          <h2 className="text-lg font-bold text-gray-900 mb-6">Qualité de la collaboration</h2>
            ...
            {ratingQuestions.map((question, qIndex) => (
              <div key={question.id} className="pb-6 border-b border-gray-100 last:border-0 last:pb-0">
                <p className="text-gray-800 font-medium mb-1">
                  <span className="text-amber-600 mr-2">{qIndex + 1}.</span>
                  {question.question}
                </p>
        ...
        <div className="flex justify-end">
          <button
            ...
            className="px-8 py-3 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
          >
            {submitting ? (
              <>
                <LoadingSpinner />
