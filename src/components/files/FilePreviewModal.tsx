...
export default function FilePreviewModal({
  ...
  // Charger le contenu du fichier (seulement pour fichiers texte/HTML)
  useEffect(() => {
    if (isOpen && file && !isBinaryFile) {
      loadFileContent();
    } else if (isOpen && file && isBinaryFile) {
      // Pour les fichiers binaires, pas besoin de charger le contenu
      setIsLoading(false);
    }
  }, [isOpen, file, isBinaryFile]);
  ...
  const loadFileContent = async () => {
    ...
    try {
      ...
      if (isHtmlFile && file.storagePath) {
        ...
        if (response.ok) {
          ...
          // Essayer de charger directement depuis le storage
        ...
        // Fichier texte (non binaire) avec URL publique ou chemin de stockage
        const fileUrl = file.publicUrl
          ? toProxyUrl(file.publicUrl) || file.publicUrl
          : pathToProxyUrl(file.storagePath);
  ...
  // URL du fichier pour affichage direct (images, PDFs)
  const getFileUrl = () => {
    if (file.publicUrl) {
      return toProxyUrl(file.publicUrl) || file.publicUrl;
    }
    if (file.storagePath) {
      return pathToProxyUrl(file.storagePath);
    }
    return null;
  };
      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* Télécharger PDF (pour fichiers HTML) */}
            {isHtmlFile && (
              <button
            ...
            {/* Télécharger fichier (pour images, PDFs, etc.) */}
        ...
        <div className="flex-1 overflow-hidden">
          ...
          {isImageFile ? (
            fileUrl ? (
              <div className="h-full flex items-center justify-center p-6 bg-gray-50 dark:bg-gray-800/50">
                <img
                  src={fileUrl}
              ...
              <div className="flex items-center justify-center h-full py-12">
                <div className="text-center">
                  ...
                  <p className="text-gray-500 dark:text-gray-400">Aperçu de l&apos;image non disponible</p>
                </div>
              </div>
            )
          ) : isPdfFile ? (
            fileUrl ? (
              ...
              <div className="flex items-center justify-center h-full py-12">
                <div className="text-center">
                  ...
                  <p className="text-gray-500 dark:text-gray-400">Aperçu du PDF non disponible</p>
                </div>
              </div>
            )
          ) : isBinaryFile ? (
            /* Autres fichiers binaires (Word, Excel, PowerPoint) */
            <div className="flex items-center justify-center h-full py-12">
              <div className="text-center">
                <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                  <svg className="w-8 h-8 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <p className="text-gray-500 dark:text-gray-400 mb-2">Aperçu non disponible pour ce type de fichier</p>
                <p className="text-xs text-gray-400 dark:text-gray-500">{file.mimeType}</p>
                {fileUrl && (
                  <button
                    onClick={handleDownload}
                    className="mt-4 inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-brand-500 rounded-lg hover:bg-brand-600 transition-colors"
                  >
                    <DownloadIcon />
                    Télécharger le fichier
                  </button>
